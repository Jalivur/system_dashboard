...
**[README.md](README.md)** ⭐ — Documentación completa v4.2. **Empieza aquí.**
...
### 💡 Ideas y Expansión
**[IDEAS_EXPANSION.md](IDEAS_EXPANSION.md)** — Roadmap v4.3+, backlog, cobertura por módulo.
...
### 📋 Config Logging (v4.2)
- Controla niveles de logging en runtime desde la pestaña **Registros**.
- Fichero, consola e individual por módulo — persistidos en `local_settings.py`.
- Útil para depurar un módulo concreto sin llenar el log de todo el sistema.

### 󰖩 Selector Interfaz WiFi (v4.2)
- Visible en el header de **Monitor WiFi** solo si hay más de una interfaz `wlan*`.
- Cambio en caliente sin reiniciar el dashboard.
- Interfaz elegida persistida en `local_settings.py`.
...
## 🗂️ Estructura de documentos v4.2

```
📚 Documentación/
├── README.md                         ⭐ Principal v4.2
...
| WiFi no lista interfaces | Verificar que existen interfaces `wlan*` en `/proc/net/dev` |
...
- `core/weather_service.py` — `WeatherService`
- `core/i2c_monitor.py` — `I2CMonitor`
- `core/gpio_monitor.py` — `GPIOMonitor`
- `core/service_watchdog.py` — `ServiceWatchdog`
- `config/local_settings_io.py` — persistencia compartida
- `ui/windows/audio_window.py` — `AudioWindow`
- `ui/windows/weather_window.py` — `WeatherWindow`
- `ui/windows/i2c_window.py` — `I2CWindow`
- `ui/windows/gpio_window.py` — `GPIOWindow`
- `ui/windows/service_watchdog.py` — `ServiceWatchdogWindow`
- `ui/windows/log_config_window.py` — `LogConfigWindow`
